...
async function generateOutfit(req, res) {
  try {
    const { gender, style, occasion, birthday } = req.body;
    ...
    const openaiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "user",
          content: `为一位${gender}生成一套${occasion}场合的${style}风格穿搭建议。生日是${birthday}。`
        }]
      })
    });

    const data = await openaiResponse.json();
    const outfitDescription = data.choices[0].message.content;
    ...
    const imageResponse = await fetch('https://api.openai.com/v1/images/generations', {
      ...
      body: JSON.stringify({
        prompt: `Fashion outfit: ${outfitDescription}`,
        n: 1,
        size: "512x512"
      })
    });
...
const handleGenerateOutfit = async () => {
  try {
    ...
    console.error('请求生成穿搭方案时出错:', error);
    toast({
      title: "错误",
      description: "生成穿搭方案时发生错误",
      variant: "destructive"
    });
  }
};
